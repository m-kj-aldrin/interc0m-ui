import { drag_state } from "./state/drag-state.svelte";
...
class NetworkState {
  ...
  add_out(module: ModuleState, destination: PeriphialUnion) {
    let new_out = new OutState(this, module, this._outs.length, destination);
...
class ChainState {
  ...
  edit() {
    let cv = `${this._input.cv.pid ?? "_"}:${this._input.cv.channel ?? "_"}`;
  ...
  remove_module(module: number | ModuleState) {
    ...
    if (module instanceof ModuleState) {
      let _index = this._modules.findIndex((_module) => _module == module);
...
export class ParameterState {
  ...
  constructor(
    ...
    // let test = $derived(
    //   this._value instanceof ModuleState ? 123 : this._value * 2
    // );

    $effect.root(() => {
      $effect(() => {
        if (this.module_pointer instanceof ModuleState) {
          let c = this.module_pointer.parent.index;
          let m = this.module_pointer.index;
          console.log("root eff", [c, m]);

          // console.log("root effect", this.module_pointer?.pointer);
        }
      });
    });
  }
